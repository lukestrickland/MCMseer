  ...
  # log-likelihood of subject parameters ps under population distribuiton p.prior  
{
  suppressWarnings(
    apply(
      sapply(seq_len(nrow(ps)), function(s) {
        p.prior_s <- assign.pp(pp,p.prior,s)
        log.prior.dmc(ps[s,],p.prior=p.prior_s)
      }), 2, sum)) 
}
...
# 
# 
# nmc=10;p.prior=p.priorDM;pp.prior=pp.prior; data = datas.modelDM
# 
  ...
  # Setup for hierarchical sampling
{
  ...
  get_phi_index <- function(p.names) 
    # get index of intercept and effects in phi  
  {
    types <- unlist(lapply(strsplit(p.names,"_"),function(x){x[[1]]}))
    tmp <- c(1:length(p.names))
    names(tmp) <- types
    first <- tmp[!duplicated(types)]
    tnames <- unique(types)
    epos <- vector(mode="list",length(tnames))
    names(epos) <- tnames
    for (i in tnames) {
      betas <- tmp[duplicated(types)]
      epos[[i]] <- betas[names(betas)==i] 
    }
    epos <- epos[unlist(lapply(epos,function(x){length(x)!=0}))]
    list(first=first,epos=epos)
  }
  ...
  if ( is.null(pp.prior) ) { # non-hierarcical
    ...
    if ( length(pp.prior[[1]])!=length(pp.prior[[2]]) )
      stop("Hyper-prior must have as many location as scale parameters")
    
    p.types <- names(pp.prior[[1]])
    ...
    if ( !is.null(hstart.prior) ) { # check hstart.prior
        stop("Doesnt work with hstart.prior!") 
    }
    if ( is.null(samples) ) { # setup new samples
      ...
      # MAYBE USE IN POSTPROCESS?
      attr(phi, "DMs") <- attr(data, "DMs")
      attr(phi,"indx") <- get_phi_index(p.names)

      # Used in assign.pp
      attr(p.prior, "DMs") <- attr(data, "DMs")
      attr(p.prior, "mu_indx") <- attr(phi,"indx")
      
      n.sig <- length(pp.prior[[1]])
      sig <- array(NA,c(n.chains,n.sig,nmc))
      dimnames(sig)[[2]] <- p.types
      
      # Parameters with effects
      p.es <- names(attr(phi,"indx")$epos)
      spt.names <- as.vector(outer(s.names,p.es,paste,sep="_"))
      alpha <- array(NA,c(n.chains,n.subjects*length(p.es),nmc))
      dimnames(alpha)[[2]] <- spt.names
      
      attr(alpha,"indx") <- matrix(1:(n.subjects*length(p.es)),nrow=n.subjects)
      colnames(attr(alpha,"indx")) <- p.es
      attr(p.prior, "alpha_indx") <-  attr(alpha,"indx")
      
      phi <- list(mu=phi,sigma=sig,alpha=alpha)  # location, scale, subject effects
      
      #### set effect size prior scales if not specified by user
      
      # condition effects
      if (is.null(attr(pp.prior[[1]], "scale_es_cond"))) {
        attr(pp.prior[[1]], "scale_es_cond") <-
          rep(0.5, length(attr(phi$mu, "indx")$epos))
        names(attr(pp.prior[[1]], "scale_es_cond")) <- 
          names(attr(phi$mu, "indx")$epos)
      }
      
      # subject effects
      if (is.null(attr(pp.prior[[1]], "scale_es_subj"))) {
        attr(pp.prior[[1]], "scale_es_subj") <- 
          rep(1, length(attr(phi$mu, "indx")$epos))
        names(attr(pp.prior[[1]], "scale_es_subj")) <- 
          names(attr(phi$mu, "indx")$epos)
      }
      ...
      if ( is.null(phi1) ) {
        ...
        for( i in 1:n.chains ) { # ASSUMES PRIOR PRODUCES GOOD VALUES !!!
          ...
          if ( is.null(hstart.prior) ) { # sample from prior
            phi[[1]][i,attr(phi[[1]],"indx")$first,1] <- 
              rprior.dmc(pp.prior[[1]])[,p.types]
            # Get effect size from standard normal so not Cauchy crazy
            for (j in names(attr(phi[[1]],"indx")$epos)) 
              phi[[1]][i,attr(phi[[1]],"indx")$epos[[j]],1] <- 
                rnorm(length(attr(phi[[1]],"indx")$epos[[j]]))
            phi[[2]][i,,1] <- 
              rprior.dmc(pp.prior[[2]])[,p.types]
            # Get effect size from standard normal so not Cauchy crazy
            phi[[3]][i,,1] <- rnorm(n.subjects*length(p.es)) 
          } else {                        # sample from hstart.prior
            phi[[1]][i,attr(phi[[1]],"indx")$first,1] <- 
              rprior.dmc(hstart.prior[[1]])[,p.types]
            # THIS COULD BE SMARTER!
            for (j in names(attr(phi[[1]],"indx")$epos)) 
              phi[[1]][i,attr(phi[[1]],"indx")$epos[[j]],1] <- 
                rnorm(length(attr(phi[[1]],"indx")$epos[[j]]))
            phi[[2]][i,,1] <- 
              rprior.dmc(hstart.prior[[2]])[,p.types] 
            # THIS COULD BE SMARTER!
            phi[[3]][i,,1] <- rnorm(n.subjects*length(p.es)) 
          }
        }
        ...
        phi[[2]][,p.types,1] <- phi1[[2]][,p.types]
        phi[[3]][,,1] <- phi1[[3]][,spt.names]
      }
      for (i in 1:n.chains) {
        h_summed_log_prior[1,i] <- 
        # evaluate prior on intercepts
        sum(log.prior.dmc(structure(phi$mu[i,attr(phi$mu, "indx")$first,1],
                                    .Names = p.types),
                          pp.prior[[1]])) + 
        # evaluate prior on standard deviations
        sum(log.prior.dmc(phi$sigma[i,,1],pp.prior[[2]])) +
        # evaluate prior on condition effects
        sum(sapply(names(attr(phi$mu, "indx")$epos),
                   function(x) {
                     deltas <- phi$mu[i,attr(phi$mu, "indx")$epos[[x]],1]
                     scale <- attr(pp.prior[[1]], "scale_es_cond")[[x]]
                     dmvc(deltas, mu = rep(0, length(deltas)),
                                         S = scale * diag(length(deltas)),
                                         log = TRUE)
                   })) +
        # evaluate prior on subject effects
        sum(sapply(colnames(attr(phi$alpha, "indx")),
                   function(x) {
                     deltas <- phi$alpha[i,attr(phi$alpha, "indx")[,x],1]
                     scale <- attr(pp.prior[[1]], "scale_es_subj")[[x]]
                     dmvc(deltas, mu = rep(0, length(deltas)),
                                         S = scale * diag(length(deltas)),
                                         log = TRUE)
                   }))
      ...
      # MAKE P.PRIOR A LIST WITH VALUES SAMPLED FROM HYPER
      ...
      for( i in 1:n.chains ) {
        h_log_likelihoods[1,i] <- sum(h.log.likelihood.dmc(p.prior=p.prior,
            ps=t(data.frame(lapply(cps,function(x){x[i,]}))),
            pp=list(phi[[1]][i,,1],phi[[2]][i,,1], phi[[3]][i,,1]) ))
      }
      ...
      for (k in 1:n.chains) {
        for (j in 1:length(out)) {
          p.priork <- assign.pp(list(phi[[1]][k,,1],
                                     phi[[2]][k,,1],
                                     phi[[3]][k,,1]),
                                p.prior=out[[1]]$p.prior,
                                s = j) 
      ...
      if ( !all(is.na(remove)) ) {
        ...
        attr(samples,"hyper")$phi[[3]] <- attr(samples,"hyper")$phi[[3]][,,-remove]
...
crossover.h <- function(k,pars,use.theta,use.logprior,use.loglike,p.priors,data,
  ...
  # Data level crossover with different priors for each chain, p.priors is a list
{
  # step size
  ...
  #   together), by default one pair at a time. 
{
  ...
  if ( !is.null(hyper) ) # hierarchical sampling
  {
    ...
    use.phi <- list(hyper$phi[[1]][,,hyper$start],hyper$phi[[2]][,,hyper$start],hyper$phi[[3]][,,hyper$start])
